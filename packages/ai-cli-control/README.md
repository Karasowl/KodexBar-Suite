...
`kodexbar-quotas` is the widget's default local command. It reads the enabled providers from `~/.config/codexbar/config.json`. Claude, Codex, Grok, Cursor, and Hermes are fetched natively with stdlib HTTP:
