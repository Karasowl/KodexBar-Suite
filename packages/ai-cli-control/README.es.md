...
El comando exacto `cost --format json --json-only` conserva el payload completo de upstream como ancla de 24 horas en `~/.cache/kodexbar-suite/cost.json`. Su refresco de 15 minutos lee solo los bytes añadidos en `~/.codex/sessions/**/*.jsonl` y `~/.claude/projects/**/*.jsonl`, con offsets y agregados por archivo en `cost-incremental.json`. Las tarifas por modelo se ajustan desde el ancla, no se codifican en este proyecto. Los modelos desconocidos aportan tokens sin costo. Un estado incremental dañado o un resultado inconsistente sirven el ancla sin cambios. Cuando el ancla cumple 24 horas, se devuelve de inmediato mientras un único scan contenido y de baja prioridad la reemplaza en segundo plano. Las demás variantes de argumentos de `cost` se reenvían a upstream. Si upstream no está instalado, el comando exacto devuelve `[]`.
