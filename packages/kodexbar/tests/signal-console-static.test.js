...
assert.match(compactRep, /compactContentWidth/, "the panel representation still knows the width of every provider block")
assert.match(compactRep, /Layout\.preferredWidth:\s*compactContentWidth/, "the panel asks for the strip width when there is room")
assert.match(compactRep, /Layout\.minimumWidth:\s*compactFloorWidth/, "a crowded panel can shrink the applet instead of overlapping the tray")
assert.doesNotMatch(compactRep, /Layout\.minimumWidth:\s*compactContentWidth/, "the applet must not demand the full strip as its minimum width")
assert.doesNotMatch(compactRep, /width:\s*Math\.max\(compactContentWidth,\s*parent\.width\)/, "the panel card must not paint wider than the slot it was given")
assert.match(compactRep, /clip:\s*true/, "the panel card clips chips that do not fit")
assert.match(compactRep, /fitWidth:\s*width/, "the strip fits itself to the width Plasma actually assigned")
assert.match(compactRep, /count > 4 && glance > 18/, "more than four providers ask for one number each instead of every quota window")
assert.match(compactStrip, /readonly property bool crowded:/, "the short panel form does not switch on and off with the assigned width")
assert.doesNotMatch(compactStrip, /tightenFit/, "fitting the strip must not measure itself again")
