...
- Show Grok weekly usage once in the compact tray with badge `W` (secondary slot). Do not copy weekly into the Session/primary slot. Primary-only compact selection still shows Grok weekly as `W`.
- Render Grok weekly as a single segmented bar (Build/API/Imagine composition points) with a legend, instead of independent surface bars.
- Treat a zero credit balance as absent for the empty-state check so "No usage reported" still appears.
