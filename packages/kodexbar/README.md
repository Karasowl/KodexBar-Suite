...
KodexBar Suite is a KDE Plasma 6 AI quota monitor for Linux. It provides a configurable compact summary in the panel and a complete popup for Claude Code, OpenAI Codex, Cursor, Grok, OpenCode Go, Hermes, Devin, Antigravity, and other providers returned by the [CodexBar CLI](https://github.com/steipete/CodexBar). Hermes remaining credits come from the local Nous Portal login. Devin remaining percents come from the local Devin login. It also includes an optional local-model viewer and a cross-provider skill inventory.
...
- Shows one provider account at a time, ordered as Codex, Claude, Grok, Antigravity, Cursor, Hermes, Devin, then every other enabled provider.
- Keeps repeated accounts separate with stable non-sensitive ordinals in tabs and compact output.
- Uses the supplied Codex, Claude, Grok, Cursor, Hermes, Devin, Antigravity, and Gemini SVG assets while keeping Antigravity and Gemini distinct.
...
- Defaults to `codex,claude,grok,antigravity,opencodego,hermes,devin`.
...
| Compact providers | Display-only ordered comma-separated provider IDs used by the system tray. The visible provider chips cover returned providers such as Codex, Claude, Grok, Antigravity, Cursor, OpenCode Go, Hermes, and Devin. Empty shows every returned provider and never filters the popup. |
